...
if (SOCIALBROWSER.fakeview.referer && SOCIALBROWSER.fakeview.referer.url) {
    referer_hostname = new URL(SOCIALBROWSER.fakeview.referer.url).hostname;
    url_hostname = new URL(SOCIALBROWSER.fakeview.url).hostname;
}
...
`;
    ...
    if (true) {
        ...
        setTimeout(() => {
            findGoodLink();
        }, 1000 * (Math.floor(Math.random() * 30) + 10));
